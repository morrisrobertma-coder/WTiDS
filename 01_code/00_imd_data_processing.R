...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Drop decile columns
dt_25 <- dt_25[, imd_decile_25 := NULL]
dt_19 <- dt_19[, imd_decile_19 := NULL]
dt_15 <- dt_15[, imd_decile_15 := NULL]
...
#~~~~~~~~~~~~~~~~~~~~~~~~~
dt_imd <- merge(dt_25,
                dt_19[,.(lsoa_code, imd_rank_19)],
                by = 'lsoa_code',
                all = T)

dt_imd <- merge(dt_imd,
                dt_15[,.(lsoa_code, imd_rank_15)],
                by = 'lsoa_code',
                all = T)

dt_imd <- merge(dt_imd,
                dt_10[,.(lsoa_code, imd_rank_10)],
                by = "lsoa_code",
                all = T)

dt_imd <- merge(dt_imd,
                dt_07[,.(lsoa_code, imd_rank_07)],
                by = "lsoa_code",
                all = T)
...
# Export
write.csv(dt_imd, file = paste0(inp_path_imd, "00_imd_data_clean.csv"))
